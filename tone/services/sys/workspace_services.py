...
from tone.core.common.job_result_helper import get_job_state
        ...
        for day_query in day_querys:
            if day_query.test_result:
                today_query_list.append({
                    ...
                    'today_query_state':
                        get_job_state(day_query.id, day_query.test_type, day_query.state, func_view_config, '', 1),
...
def get_day_querys(data, now, product_id, project_id, hours_24_ago):
    if data.get('date'):
        ...
        if data.get('hours_24_ago'):
            gte_time = now - timedelta(days=1)
        elif data.get('hours_48_ago'):
            gte_time = now - timedelta(days=2)
        elif data.get('seven_day_ago'):
            gte_time = now - timedelta(days=7)
        else:
            return []
        day_querys = TestJob.objects.filter(product_id=product_id, project_id=project_id,
                                            gmt_created__gte=gte_time,
